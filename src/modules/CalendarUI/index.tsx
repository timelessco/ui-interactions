...
export const CalendarIcon = (props: CalendarIconProps) => {
  const disabledColor = tailwind.color("bg-gray-500") as string;
...
export const CalendarUI = () => {
  const { selectedDate, isMomentumScrollBegin } = useCalendarContext();
  const handlePress = () => {
    selectedDate.value = dayjs().format("YYYY-MM-DD");
  };
  const derivedMomentumState = useDerivedValue(() =>
    isMomentumScrollBegin ? withSpring(1) : withSpring(0),
  );
  return (
      ...
      >
        ...
        <Pressable onPress={handlePress}>
          <Animated.View style={[tailwind.style("p-2 shadow-xl z-50")]}>
            <CalendarIcon state={derivedMomentumState} />
          </Animated.View>
        </Pressable>
      </Animated.View>
      <WeekStrip />

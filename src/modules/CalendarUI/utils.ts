...
/**
 * The function calculates and returns different date ranges based on input parameters.
 * @param {number} initialFirstDay - The initial day of the week, represented as a number from 0 to 6
 * (0 being Sunday and 6 being Saturday).
 * @param {string} minDateStr - The minimum date in string format, in the format "YYYY-MM-DD".
 * @param {string} maxDateStr - `maxDateStr` is a string representing the maximum date allowed in the
 * date range. It is used in the `calculateDates` function to calculate the total number of days in the
 * date range and to determine the end date for each type of date grouping (day, week, threeDays, work
 * @param {string} initialDate - `initialDate` is a string representing a date in the format
 * "YYYY-MM-DD". It is used as a reference point to determine the index of the date in the arrays
 * returned by the `calculateDates` function.
 * @returns an object with four properties: `day`, `week`, `threeDays`, and `workWeek`. Each property
 * is an object with two properties: `data` and `index`. The `data` property is an array of date
 * strings, and the `index` property is an integer representing the index of the initial date string in
 * the `data` array.
 */
